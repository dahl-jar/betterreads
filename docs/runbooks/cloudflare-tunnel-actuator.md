...
The Spring Boot management port is bound to `127.0.0.1:8081` on the OCI VM. That's correct for security but useless if you can't read the metrics from your laptop. This runbook covers the deploy-day work that gets `actuator/health`, `actuator/health/liveness`, `actuator/health/readiness`, and `actuator/prometheus` reachable from a browser, but only after Cloudflare Access authenticates the request.

## Prerequisites that have to exist before you start

A domain on Cloudflare DNS. The cheapest way to get one is Cloudflare Registrar at the wholesale price, around $10 a year for a `.com`. Any other registrar works too as long as the domain's nameservers point at Cloudflare's. Until a real domain exists, this runbook is theoretical.

The OCI VM provisioned and reachable over SSH, with `cloudflared` installed. The cloud-init script under `infra/terraform/cloud-init/` doesn't install `cloudflared` yet; add it the day the VM comes up, or update the script first.

The Spring Boot app deployed and listening on the VM with `management.server.port=8081` bound to `127.0.0.1`. Confirm with `curl http://127.0.0.1:8081/actuator/health` from inside an SSH session before doing anything Cloudflare-side.

## Architecture

The browser hits `https://metrics.betterreads.example.com/actuator/prometheus`. Cloudflare's edge intercepts that hostname because the DNS record points at Cloudflare. Before forwarding the request, Cloudflare Access requires a Google SSO login from the configured email. After auth, Cloudflare attaches a `Cf-Access-Jwt-Assertion` header and forwards through the tunnel to the VM. The tunnel hands the request to `127.0.0.1:8081`. Spring Boot Actuator answers.

The tunnel runs as a service on the VM and connects outbound to Cloudflare's edge over QUIC. There are no inbound firewall holes; the OCI security list keeps port 8081 closed to the world, which is the point.
...
Install `cloudflared` if cloud-init hasn't already:
...
Drop a config file at `/etc/cloudflared/config.yml` based on the template at `infra/cloudflare/tunnel-config.yml.example` in this repo. Fill in the tunnel ID and the credentials path. The ingress rules route `metrics.<domain>` to the management port and let everything else fall through to a 404.

Route DNS at the metrics hostname:

```bash
cloudflared tunnel route dns betterreads metrics.<your-domain>
```
...
```bash
curl https://metrics.<your-domain>/actuator/health
```
...
curl -H "CF-Access-Client-Id: <id>.access" \
     ...
     https://metrics.<your-domain>/actuator/health
```
...
If the tunnel goes down, the metrics URL returns a Cloudflare error page (525 or 1033, depending on what failed). The actual API on `https://api.<your-domain>` keeps working because it's a different tunnel ingress rule (or a different tunnel entirely). The two are independent.
...
Multi-region failover. The OCI Free Tier is one region. If `eu-amsterdam-1` is down, metrics are unreachable. Acceptable cost for the project's scale.
