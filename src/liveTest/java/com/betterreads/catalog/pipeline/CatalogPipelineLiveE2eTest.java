...
/** Runs the six-book slate through live sources, staging, promotion, and a local catalog read. */
...
class CatalogPipelineLiveE2eTest extends ContainerizedTest {
    ...
    @Test
    @DisplayName("the live slate reaches the catalog and reads back within 200 ms")
    void liveSlateReachesCatalogAndReadsBackWithinLimit() {
    ...
    private void assertLocalReadWithinLimit() {
        ...
        assertThat(elapsed)
            .as("the promoted catalog read finishes within the local-read limit")
            .isLessThan(MAX_READ);
    }
