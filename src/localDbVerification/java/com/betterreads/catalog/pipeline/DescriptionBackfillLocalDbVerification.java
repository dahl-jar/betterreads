...
/** Rechecks seeded top-rated descriptions with live sources and local Postgres. */
...
class DescriptionBackfillLocalDbVerification {
    ...
    private static final String SEEDED_DESCRIPTION_REQUIRED = "seeded book has a description";
    ...
    @Test
    @DisplayName("the full sweep replaces foreign, fact, and marketing descriptions")
    void sweepReplacesForeignFactAndMarketingDescriptions() {
        seedTopRatedBooks();

        backfillService.fullSweep();

        for (final Book book : books.findAll()) {
            final String currentDescription = Objects.requireNonNull(
                book.getDescription(), SEEDED_DESCRIPTION_REQUIRED);
            LOG.info("catalog.localdb.description {} | {}",
                LogSanitizer.forLog(book.getTitle()),
                LogSanitizer.forLog(head(currentDescription)));
        }
    ...
    private void seed(final SourceBook book) {
        bookUpsertService.upsertFromSource(book);
    }
    ...
    private String description(final String isbn) {
        final Book book = books.findAll().stream()
            .filter(candidate -> isbn.equals(candidate.getIsbn()))
            .findFirst()
            .orElseThrow();
        return Objects.requireNonNull(book.getDescription(), SEEDED_DESCRIPTION_REQUIRED);
    }
