...
/**
 * HMAC-SHA256 hasher for password-reset tokens. Keyed by the JWT secret so a DB-only leak
 * cannot reconstruct active tokens. Mirrors the refresh-token hasher rather than sharing it
 * because the two domains have different lifetimes and revocation rules; collapsing them now
 * would force one class to grow conditionals as soon as either side changes.
 */
@Component
final class PasswordResetTokenHasher {
    ...
    PasswordResetTokenHasher(final JwtProperties properties) {
